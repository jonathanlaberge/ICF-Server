...
local racetimertime = 100 -- MATCH WITH TIME IN SERVER INIT
local speedometer_maxspeed = 300
_G['tHM'] = 0
_G['tLastRace'] = 0
_G['HM'] = 0
_G['lastRace'] = 0
	...
	for k, v in pairs(player.GetAll()) do
		...
		elseif (myself:Team() == TEAM_RACING && v:Team() == TEAM_RACING) then
			...
			if v:InVehicle() then
				...
				--plyspeed = "("..plyspeedisgreater..plyspeeddelta..") "..plyspeed.." km/h"
			else
				plyspeed = "N/A"
				plyspeeddelta = 0
			end
			...
			draw.SimpleTextOutlined(v:Name(), "Trebuchet18", pos.x, pos.y, thecolor, 1, 1, 1, thecolor2 )
			draw.SimpleTextOutlined("".. distance .." FT", "Trebuchet18", pos.x, pos.y + 16, thecolor, 1, 1, 1, thecolor2 )
			draw.SimpleTextOutlined(tostring(plyspeed).." km/h", "Trebuchet18", pos.x, pos.y + 16*2, thecolor, 1, 1, 1, thecolor2 )
			...
			draw.SimpleTextOutlined("( ".. tostring(plyspeedisgreater..plyspeeddelta.." )"), "Trebuchet18", pos.x, pos.y + 16*3, thecolor, 1, 1, 1, thecolor2 )
		end
	end
end
...
function Leeroy()	
	...
	if math.Round(NextRace) < 10 then  
	...
	if math.Round(NextRace) < 10 then  
		slbnextracecolor = Color(22, 150, 242, 255) 
	end
	draw.DrawText(math.Round(NextRace),"Trebuchet18",slbclockx-26,slbclocky+64*slbclockz + 8,slbnextracecolor,1)
	...
	// Player Name
	draw.DrawText("Name: " .. Name,"Trebuchet18",13,10,Color(255, 255, 255, 255),0,0)

	// Stats
	draw.DrawText("Top Three in " .. Won .. " out of " .. Won + Lost .. " races","Trebuchet18",13,30,Color(255, 255, 255, 255),0,0)

	// Building Racing
	draw.DrawText("Racing: " .. HowManyRacing .. " of " .. TotalPlayers .." players","Trebuchet18",13,50,Color(255, 255, 255, 255),0,0)

	// Last Race Title
	draw.DrawText("Race #" ..HM.. " Results:","Trebuchet18",13,70,Color(255, 255, 255, 255),0,0)

	// Last Race Stats
	draw.DrawText("1st: " .. FirstPosition,"Trebuchet18",13, 90,Color(0, 255, 0, 255),0,0)
	draw.DrawText("2nd: " .. SecondPosition,"Trebuchet18",13, 110,Color(255, 255, 0, 255),0,0)
	draw.DrawText("3rd: " .. ThirdPosition,"Trebuchet18",13, 130,Color(255, 0, 0, 255),0,0)
	
	if ( IsValid( LocalPlayer()) and LocalPlayer():InVehicle() ) then
		local conv = 17.6 // km/h convertion value
	...
	draw.DrawText("(" .. currenttopspeed .. " km/h)","Trebuchet18",slbspeedx,slbspeedy + 64 + 16,Color(150, 150, 150, 255),1)
	draw.DrawText("" .. speedstr .. " km/h","Trebuchet18",slbspeedx,slbspeedy + 64 + 16+12,Color(150, 150, 150, 255),1)
