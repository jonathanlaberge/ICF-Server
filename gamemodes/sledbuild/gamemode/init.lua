...
function NewRound()	
	...
	for k,v in pairs(ents.FindByClass("player")) do
		...
		if (v:InVehicle() && v:GetNWBool( "OnTrack" ) == true )then
			...
			v:PrintMessage(HUD_PRINTTALK, "[Sledbuild] You did not finish the race in time. Try to make your sled faster!")
			if Jonathan1358LibLoaded then
				net.Start("Jonathan1358.Msg.CenterBox")
					net.WriteString("You did not finish the race in time.\nTry to make your sled faster!")
					net.WriteFloat(225)
					net.WriteFloat(0)
					net.WriteFloat(0)
					net.WriteFloat(255)
...
function LeaveVehicle(ply, vehicle)
	ply:SetCollisionGroup( COLLISION_GROUP_WEAPON )
end

hook.Add("PlayerLeaveVehicle","LeaveVehicle",LeaveVehicle)

function PhysgunPickup(plr, prop)
	prop:SetCollisionGroup(COLLISION_GROUP_DEBRIS_TRIGGER)
end
