...
function GB_QueryDatabaseForServer()
	...
	local HeartbeatQuery = ULX_DB:query("SELECT ServerID FROM ulx_servers WHERE IPAddress ='"..IPAddress.."' AND Port = '"..HostPort.."'");
...
function GB_UpdateServerName() 
	...
	local UpdateName = ULX_DB:query(" UPDATE ulx_servers SET HostName='".. HostName .."' WHERE ServerID='"..GB_SERVERID.."' ");
...
function GB_InsertNewServer()
	...
	local NewServer = ULX_DB:query("INSERT INTO ulx_servers VALUES ('','"..IPAddress.."','"..HostPort.."','"..HostName.."')");
