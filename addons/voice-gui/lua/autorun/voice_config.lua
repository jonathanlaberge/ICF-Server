...
VOICEVIS.NotFriendColor = Color(120,120,120)

-- The color of the border around a friend
VOICEVIS.FriendColor = Color(0,250,250)
...
function VOICEVIS:GetColorForPlayer(ply)
	if ply:IsAdmin() then
		return HSVToColor( math.abs(math.sin(0.3*RealTime())*128), 1, 1 )
	end
end
...
VOICEVIS.VisualizerSetting[1] = {
	...
	MULTIPLIER = 100 -- Multiplier to your voice
}
...
VOICEVIS.VisualizerSetting[6] = {
	WIDTH = 5, -- Width of each line segment
	SPACING = -1, -- Space between each line segment
	MULTIPLIER = 10 -- Multiplier for player voice
}